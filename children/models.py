...
Схема трёх таблиц: children_child, children_parent и связь
children_parentchild. Ограничение на наполняемость группы проверяется в
Clean(): форма выдаёт ошибку при попытке зачислить ребёнка сверх лимита.
"""
...
class Child(models.Model):
    ...
    def clean(self) -> None:
        ...
        Реализует функциональное требование: система должна автоматически
        проверять лимит наполняемости группы и выдавать валидационную
        ошибку при попытке зачисления сверх этого лимита.
        """
        super().clean()
