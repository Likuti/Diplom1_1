...
Проверяем основные сценарии работы с посещаемостью:
- идемпотентное сохранение через update_or_create при POST-запросе формы;
- разграничение доступа: воспитатель видит только свою группу.
"""
